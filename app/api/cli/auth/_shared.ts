...
const userCodeSchema = z.object({
  user_code: z.string().regex(USER_CODE_PATTERN),
});
...
export async function validateDeviceAuthAction(
  ...
  if (new Date() > record.expires_at) {
    // Guard with status='pending' to prevent overwriting a concurrent approve/deny
    await db.update(deviceAuthCodesTable)
      .set({ status: 'expired' })
      .where(and(eq(deviceAuthCodesTable.uuid, record.uuid), eq(deviceAuthCodesTable.status, 'pending')));
    return {
      ok: false,
      response: createErrorResponse('Authorization code expired', 410, 'EXPIRED'),
    };
