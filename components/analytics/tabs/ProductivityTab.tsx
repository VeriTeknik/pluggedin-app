...
export function ProductivityTab({ profileUuid, period }: ProductivityTabProps) {
  ...
  const safeMetrics = {
    activeStreak: metrics.activeStreak || 0,
    mostProductiveHour: metrics.mostProductiveHour || 0,
    mostProductiveDay: metrics.mostProductiveDay || '',
    avgToolCallsPerDay: metrics.avgToolCallsPerDay || 0,
    avgDocumentsPerWeek: metrics.avgDocumentsPerWeek || 0,
    toolCombinations: metrics.toolCombinations || [],
    ...
    <>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <MetricCard
          ...
          value={safeMetrics.activeStreak}
          icon={Calendar}
          description={t('productivity.streakDays', { count: safeMetrics.activeStreak })}
        />
        <MetricCard
          title={t('productivity.mostProductiveHour')}
          value={`${safeMetrics.mostProductiveHour}:00`}
          icon={Clock}
        />
        <MetricCard
          title={t('productivity.avgToolCallsPerDay')}
          value={safeMetrics.avgToolCallsPerDay.toFixed(1)}
          icon={Zap}
        />
        <MetricCard
          title={t('productivity.avgDocumentsPerWeek')}
          value={safeMetrics.avgDocumentsPerWeek.toFixed(1)}
          icon={FileText}
        />
      </div>

      {/* Tool Combinations / Workflows */}
      {safeMetrics.toolCombinations.length > 0 && (
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-2">
            {safeMetrics.toolCombinations.map((combo: any, index: number) => (
        ...
        <CardContent>
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">{safeMetrics.mostProductiveDay}</span>
            </div>
            <div className="flex justify-between">
              <span>{t('productivity.mostProductiveHour')}</span>
              <span className="font-semibold">{safeMetrics.mostProductiveHour}:00</span>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="space-y-2">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {safeMetrics.achievements.map((achievement: any) => (
            <AchievementBadge
              key={achievement.id}
