...
Add a second file covering token validation itself. `authenticateConnectorRequest` decides whether a request is authenticated at all, so its expiry and revocation branches need coverage rather than being left to the route tests in Phase B.

```ts
// tests/oauth/authenticate-token.test.ts
...
Run both: `pnpm test tests/oauth/authenticate.test.ts tests/oauth/authenticate-token.test.ts`
Expected: 11 tests total once the implementation exists.
...
Run: `pnpm test tests/oauth/authenticate.test.ts tests/oauth/authenticate-token.test.ts`
Expected: PASS, 11 tests.
...
```bash
pnpm test tests/oauth/ && pnpm lint
git add lib/oauth/authenticate.ts tests/oauth/authenticate.test.ts tests/oauth/authenticate-token.test.ts
...
Expected: the pre-existing 42 failed files / 204 failed tests are unchanged, plus roughly 57 new passing tests across `tests/oauth/`. `pnpm build` succeeds. If the failure counts moved, find out why before starting Phase B.
