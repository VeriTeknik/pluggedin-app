...
-- This migration consolidates all the fixes we've applied directly to the database

-- 1. Fix conversation_memories table
ALTER TABLE conversation_memories 
ADD COLUMN IF NOT EXISTS source_ref TEXT,
ADD COLUMN IF NOT EXISTS last_used_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- 2. Fix system_logs table
ALTER TABLE system_logs
ADD COLUMN IF NOT EXISTS details JSONB DEFAULT '{}'::jsonb,
ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- 3. Fix workflow_tasks table
...
-- 4. Fix conversation_tasks table structure
...
-- 5. Convert priority column type if needed (from integer to varchar)
DO $$ 
BEGIN
...
-- 6. Fix task_type column to be nullable with default
ALTER TABLE conversation_tasks 
ALTER COLUMN task_type DROP NOT NULL,
ALTER COLUMN task_type SET DEFAULT 'task';

-- 7. Make workflow_id nullable as it's not always provided
ALTER TABLE conversation_tasks 
ALTER COLUMN workflow_id DROP NOT NULL;

-- 8. Fix workflow_templates table
...
-- 9. Fix workflow_executions table
